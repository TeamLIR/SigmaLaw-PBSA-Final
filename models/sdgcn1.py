...
class SDGCN(nn.Module):
    def __init__(self, bert, opt):
        ...
        self.dense = nn.Linear(opt.hidden_dim *8 , opt.polarities_dim)

    def forward(self, inputs):
        context, aspect, all_aspects, aspects_num_max, all_targets_position, targets_all ,x_len = inputs[0], inputs[1], inputs[2], self.opt.aspect_num_max, inputs[3], inputs[4] ,inputs[5]
        ...
        batch_size = 16
        # batch_size = torch.shape(self.input_x)[0]
        targets_all_len = []
        for i in range(aspects_num_max):
             # targets_i_len = torch.narrow(self.targets_all, [0, i], [batch_size, 1])
             targets_i_len = targets_all[: ,i ,:]
        ...
        context, (_, _) = self.lstm_context(context, torch.tensor([80]))
        #print("context: " , context.size())
        ...
        for i in range(aspects_num_max):
            ...
            att_s_i= self.attention_aspect(LSTM_aspects_all[i], context_pool, targets_all_len[i])
            ...
            #print(target_position_i.type())
            #print("context" ,context.size())
            #print(context_pool.double().type())
            #print("target_position_i" ,target_position_i.size())
            LSTM_Hiddens_sen_i = torch.mul(context ,target_position_i )
            #print("LSTM_Hiddens_sen_i" ,LSTM_Hiddens_sen_i.size())

            att_s_i = self.attention_aspect(LSTM_Hiddens_sen_i.float(), outputs_ss[i],  x_len)
            outputs_ts[i] = torch.squeeze(torch.matmul(att_s_i.float(), LSTM_Hiddens_sen_i.float()), axis=1)
            #print("squeeze", outputs_ts[i].size())

        # for i in range(len(outputs_ts)):
        #      outputs_ts[i]=torch.unsqueeze(outputs_ts[i], axis=2)
        #print("unsqueeze", outputs_ts[1].size())
        x = torch.cat((outputs_ts[0] ,outputs_ts[1] ,outputs_ts[2] ,outputs_ts[3] ,outputs_ts[4] ,outputs_ts[5]
                       ,outputs_ts[6] ,outputs_ts[7]), dim=-1)
        #print("x" ,x.size())
        out = self.dense(x)
        return out
